...
class XbeeControl:
    def __init__(self):
        ...
        if self.XBEE_ENABLE:
            self.PORT = CONSTANTS.COMMUNICATION.DEFAULT_PORT  # change based on current xbee coms
            self.BAUD_RATE = CONSTANTS.COMMUNICATION.DEFAULT_BAUD_RATE  # change based on xbee baud_rate
            self.xbee_device = XBeeDevice(self.PORT, self.BAUD_RATE)
            self.xbee_device.open()
            self.remote_xbee = RemoteXBeeDevice(
                self.xbee_device, XBee64BitAddress.from_hex_string(CONSTANTS.COMMUNICATION.REMOTE_XBEE_ADDRESS)
            )
        ...
        self.DEADBAND = CONSTANTS.TIMING.DEADBAND_THRESHOLD  # this is the dead band on the controller
        self.FREQUENCY = CONSTANTS.TIMING.UPDATE_FREQUENCY  # how often the message is sent, (ns)
        self.updateLoop = 0
    ...
    def SendJoystickAxis(self, newEvent: Event):
        ...
        if self.reverseMode:
            ...
            multiplier = CONSTANTS.CONTROLLER_MODES.REVERSE_MULTIPLIER if not self.creepMode else -CONSTANTS.CONTROLLER_MODES.CREEP_MULTIPLIER
    ...
    def UpdateInfo(self):
        ...
        # Handle heartbeat if XBee is enabled
        if self.XBEE_ENABLE and hasattr(self, 'heartbeat_manager'):
