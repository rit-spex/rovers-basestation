...
def test_native_to_int_bytes_direct_call():
    # Verify that the internal conversion function also applies validation
    comm = MessageEncoder()
    sig = comm.get_messages()[CONSTANTS.COMPACT_MESSAGES.XBOX_ID]["values"][
        CONSTANTS.XBOX.JOYSTICK.AXIS_LY_STR
    ]
    # In-range: single byte
    val = comm._native_to_int(sig, b"\xc8")
    assert val == 200

    # Out-of-range: two bytes representing 0x0100 = 256
    with pytest.raises(ValueError):
        comm._native_to_int(sig, b"\x01\x00")
