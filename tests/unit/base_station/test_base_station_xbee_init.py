...
class TestXBeeInitialization:
    ...
    @patch("xbee.app.XBEE_AVAILABLE", True)
    @patch("xbee.app.XBeeDevice")
    @patch("xbee.app.RemoteXBeeDevice")
    @patch("xbee.app.XBee64BitAddress")
    @patch("xbee.app.CommunicationManager")
    @patch("xbee.app.HeartbeatManager")
    @patch("xbee.app.ControllerManager")
    @patch("xbee.app.CONSTANTS")
    ...
    def test_xbee_initialization_falls_back_on_oserror(
        ...
        with (
            patch("xbee.app.XBEE_AVAILABLE", True),
            patch("xbee.app.XBeeDevice") as mock_device,
            patch("xbee.app.CONSTANTS") as mock_constants,
            patch("xbee.app.logger") as mock_logger,
        ):
            mock_constants.SIMULATION_MODE = False
            mock_constants.COMMUNICATION.DEFAULT_PORT = "/dev/ttyUSB0"
            mock_constants.COMMUNICATION.DEFAULT_BAUD_RATE = 9600
...
class TestXBeeErrorLogging:
    ...
    def test_log_xbee_init_error_oserror(
        ...
        with patch("xbee.app.logger") as mock_logger:
